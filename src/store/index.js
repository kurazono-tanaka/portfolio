...
const actions = {
  ...
  async getUserList ({commit}) {
    await firebase.firestore().collection('users').get().then((query) => {
      ...
      const createUserArray = buff.filter(doc => doc[2] === state.mailaddress)
  ...
  async getAllLists ({commit}) {
    await firebase.firestore().collection('development_languages').get().then((query) => {
    ...
    await firebase.firestore().collection('hobbies').get().then((query) => {
  ...
  async updateInformation ({commit}, {belongs, country, city, language, hobby, message}) {
    // DBの金額更新
    console.log('これからupdateInformation実施')
    const userDoc = firebase.firestore().collection('users').doc(state.userId)
  ...
  async getUsers ({commit}, {username, belongs, country, city, language, hobby}) {
    await firebase.firestore().collection('users').get().then((query) => {
      ...
      buff.forEach((doc) => {
        ...
        const languageDiff = doc[5].filter(item => language.indexOf(item) !== -1)
